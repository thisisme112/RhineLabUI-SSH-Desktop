...
/**
 * The offline worker is meaningless inside the app shell: every asset is
 * already on the device, the shell serves them from its own origin, and a cache
 * that outlives a `cap sync` can serve the previous release's bundle.
 */
const offline = () => !isWallpaper && !isDesktop && !isAndroid && !isNativeShell();
