...
export function synthesizeSound(
  ...
  switch (type) {
    case "ssh-open":
      glass(1380, .037, .26); tone(460, 640, .018, .13, .025, .009); break;
    case "ssh-collapse":
      glass(1060, .033, .22); tone(540, 370, .018, .15, .025, .01); break;
    case "ssh-ended":
      tone(620, 500, .024, .13, 0, .01); tone(310, 310, .017, .18, .10, .012); break;
    case "ssh-switch":
      tone(880, 920, .018, .075, 0, .007); break;
    case "ssh-auth":
      tone(740, 740, .025, .095, 0, .012);
      tone(740, 740, .016, .11, .14, .012);
      break;
    case "ssh-connected":
      tone(520, 520, .031, .15, 0, .012);
      tone(780, 780, .022, .22, .10, .014);
      break;
    case "ssh-disconnected":
      tone(540, 430, .025, .18, 0, .014);
      tone(320, 320, .019, .24, .13, .018);
      break;
    case "ssh-transfer-complete":
      tone(960, 960, .021, .10, 0, .008);
      tone(1280, 1280, .014, .16, .07, .01);
      break;
    case "ssh-transfer-failed":
      tone(470, 410, .023, .18, 0, .015);
      tone(360, 360, .017, .18, .14, .012);
      break;
    case "page-open":
...
export class TerminalAudio {
  ...
  private get musicLevel() { return this.prefs.musicVolume * (this.operating ? .45 : 1); }
  setOperating(operating: boolean) {
    if (this.operating === operating) return;
    this.operating = operating;
    if (this.context) level(this.musicBus!.gain, this.prefs.music ? this.musicLevel : 0, this.context.currentTime, .5);
  }
  ...
  configure(prefs: AudioPreferences) {
    ...
    if (this.context) {
      ...
      level(
        this.musicBus!.gain,
        this.prefs.music ? this.musicLevel : 0,
        this.context.currentTime,
        0.2,
      );
    }
  ...
  private createContext() {
    ...
    this.musicBus.gain.value = this.prefs.music ? this.musicLevel : 0;
