import { archiveColumns, columnFiles, columnRowOffset, fileLocation } from "./data.ts";
...
export function fileAtCell({ lane, row }: ArchiveCell) {
  const column = wrap(lane, archiveColumns.length);
  const files = columnFiles(column);
  return files[wrap(row - 12 + columnRowOffset(column), files.length)];
}
