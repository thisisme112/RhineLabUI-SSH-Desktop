...
import { archiveColumns, rebaseArchiveRows, resetArchiveRows, fileLocation, records } from "./data";
import {
  cellKey,
  sameCell,
  selectionCell,
...
import { ArchivePull } from "./archive-pull";
...
/**
 * A session rendered as the file's contents (see ssh/terminal-deck.ts). The
 * scene owns its placement and the open/close motion; the deck owns what its
 * screen shows. Kept as an interface so the xterm dependency stays out of
 * every bundle that has no session to show.
 */
...
export type SessionDeckState = "off" | "packed" | "open";
...
export class ArchiveScene {
  ...
  dispose() {
    ...
    this.cancelPointer();
    for (const item of this.outgoing) this.disposeReturning(item);
    this.outgoing = [];
    this.deck?.dispose();
    this.deck = null;
  ...
  setTheme(dark: boolean, immediate = false) {
    ...
    this.theme.set(dark, performance.now() / 1000, this.selectedCell, immediate);
  }
  private playfield = { enabled: false, bands: quietBands(), strength: 1, flatten: 0, target: null as string | null, breathing: true };
  private ambientMotion = true;
  setAmbientMotion(enabled: boolean) { this.ambientMotion = enabled; if (!enabled) this.idleGain = 0; }
  private flatMix = 0;
  ...
  /** When set, an external timeline owns the reveal (see setDecryptionReference). */
  private decryptionReference: number | null = null;
  ...
  private outgoing: {
    ...
    returnY: number | null;
    clarity: number;
    deck?: ReturnType<SessionDeck["snapshot"]>;
    openBlend?: number;
    /** Sub-pixel preview tails share the array batches, retaining their spring. */
    batched?: boolean;
    terminal?: boolean;
  }[] = [];
  ...
  private archivePull = new ArchivePull();
  ...
  private deck: SessionDeck | null = null;
  private openBlend = 0;
  private deckState: SessionDeckState = "off";
  private labelMesh?: THREE.Mesh;
  private deckCovered = false;
  private coveredAt = 0;
  private redrawUntil = 0;
  private renderedFrames = 0;
  private requestedDark = false;
  private archivePreview?: { index: number; cell: ArchiveCell; started: number; x: number; z: number; vx: number; vz: number; targetX: number; targetZ: number };
  onPreviewCommit?: (index: number, cell: ArchiveCell) => void;
  previewArchive(index: number) {
    if (this.targetDetail || this.archivePreview?.index === index) return;
    this.stopMomentum();
    const cell = selectionCell(index, {
      lane: Math.round(this.columnCamera.value / COLUMN_SPACING + 2),
      row: Math.round((-this.rail.value - 2.17) / ROW_SPACING + 15.5),
    });
    const target = this.cellPosition(cell);
    this.archivePreview = { index, cell, started: this.clock, x: this.columnCamera.value, z: this.rail.value,
      vx: this.columnCamera.velocity, vz: this.rail.velocity, targetX: target.x, targetZ: -2.17 - target.z };
  ...
  private disposeReturning(item: (typeof this.outgoing)[number]) {
    ...
    this.appearance.dispose(item.group);
  }

  /** Install the deck once the desktop session layer exists. */
  setSessionDeck(deck: SessionDeck) {
    this.deck?.dispose();
    this.deck = deck;
    deck.group.userData.sessionDeck = true;
    this.model.add(deck.group);
    // Selection clones read the last child as the label; keep it last.
  ...
  setSessionDeckState(state: SessionDeckState) {
    ...
    if (state === "open") { this.cancelPointer(); this.targetRotation = 0; }
  }
  ...
  private updateSessionDeck(dt: number, time: number) {
    ...
    try {
      ...
      // Original meshes remain untouched for array clones and exact return.
      for (const child of this.model.children) if (child instanceof THREE.Mesh) child.visible = !visible;
      if (visible)
        this.deck.update(
          dt,
          time,
          this.reduced,
          this.sessionDeckFocus,
          this.themeAmount,
        );
    } catch (error) {
      console.error("session deck disabled:", error);
      this.openBlend = 0;
      this.deckState = "off";
      const deck = this.deck;
      this.deck = null;
      deck.dispose();
      for (const child of this.model.children) if (child instanceof THREE.Mesh) child.visible = true;
    }
  }

  private sessionScreenSpan() {
    const screen = this.deck!.screenSize;
    const aspect = this.container.clientWidth / this.container.clientHeight;
    return Math.max(screen.height / .975, screen.width / (aspect * .985));
  }

  projectSessionScreen() {
    if (!this.deck?.group.visible) return null;
    const { width, height, center: [x, y, z] } = this.deck.screenSize;
    this.model.updateMatrixWorld(true);
    const project = (px: number, py: number): [number, number] => {
      const p = this.deck!.group.localToWorld(new THREE.Vector3(px, py, z)).project(this.camera);
      return [(p.x + 1) * this.container.clientWidth / 2, (1 - p.y) * this.container.clientHeight / 2];
    };
    const scale = this.container.clientHeight / this.sessionScreenSpan();
    return {
      corners: [project(x - width / 2, y + height / 2), project(x + width / 2, y + height / 2),
        project(x + width / 2, y - height / 2), project(x - width / 2, y - height / 2)] as const,
  ...
  setMode(mode: "hidden" | "archive" | "detail") {
    this.archivePreview = undefined;
    this.cancelPointer();
    if (mode !== "archive") this.manualExtraction = false;
    this.setHover(null);
  ...
  private rebaseCoordinates() {
    ...
    if (shift.row) rebaseArchiveRows(shift.row);
    this.setHover(null);
  ...
  select(index: number, navigation?: ArchiveNavigation) {
    ...
    if (this.looping && changed && this.loaded && this.lift.value > 0.0001) {
      const batched = this.lift.value < .06 &&
        !this.rotation && !this.openBlend && this.decryption.clarity < .001;
      const group = new THREE.Group();
      group.position.copy(this.model.position);
      group.rotation.copy(this.model.rotation);
      // Never clone a hidden terminal hierarchy just to immediately discard it.
      if (!batched) for (const child of this.model.children)
        if (!child.userData.sessionDeck) group.add(child.clone(true));
      // A returning card owns its terminal screen and label, even after the
      // active deck is rebound to a different host or functional resource.
      const snapshot = !batched && this.deck?.group.visible ? this.deck.snapshot() : undefined;
      if (!batched) {
        for (const child of group.children) if (child instanceof THREE.Mesh) child.visible = !snapshot;
      ...
      if (snapshot) group.add(snapshot.group);
      ...
      this.outgoing.push({
        ...
        lift: { ...this.lift },
        returnY: group.rotation.y !== 0 || this.openBlend > 0 ? group.position.y : null,
        clarity: this.decryption.clarity,
        deck: snapshot,
        openBlend: this.openBlend,
        batched,
        terminal: Boolean(this.deck?.group.visible),
      });
      this.lift.value = 0;
    ...
    if (returning >= 0) {
      ...
      if (o.deck || o.terminal) { this.deckState = "packed"; this.openBlend = o.openBlend ?? 0; }
  ...
  resize() {
    this.redrawUntil = this.clock + 1.5;
    this.coveredAt = 0;
  ...
  private projectDragMotion(motion: THREE.Vector3) {
    this.model.updateMatrixWorld(true);
  ...
  private dragProjection(): DragProjection {
    ...
    return {
      lane: this.projectDragMotion(new THREE.Vector3(-COLUMN_SPACING, 0, 0)),
      row: this.projectDragMotion(new THREE.Vector3(0, 0, -ROW_SPACING)),
    };
  }
  ...
  private bindPointer() {
    ...
    const moveArchive = (e: PointerEvent) => {
      if (this.archivePull.move(e.clientX, e.clientY)) {
        if (this.archivePull.active) {
          moved = true;
          this.manualExtraction = true;
          this.lastInteraction = this.clock;
    ...
    canvas.addEventListener("pointerdown", (e) => {
      ...
      const pressed = browse && !moved && this.onOpen && !this.relayActive
        ? this.pickCell(e.clientX, e.clientY) : null;
      if (pressed && sameCell(pressed, this.selectedCell) && this.detail < .01 && this.openBlend === 0) {
        // Use the camera's projected vertical, not a screen-distance shortcut.
        // Only the chosen model is pulled; the array keeps both browsing axes.
        this.lift.value += this.hoverLifts.get(cellKey(this.selectedCell)) ?? 0;
        this.lift.velocity = 0;
        this.hoverLifts.delete(cellKey(this.selectedCell));
        this.manualExtraction = true;
        this.archivePull.start(e.clientX, e.clientY,
          this.projectDragMotion(new THREE.Vector3(0, 1, 0)), this.lift.value, INSPECTION_LIFT);
      }
      this.setHover(null);
    ...
    canvas.addEventListener("pointerup", (e) => {
      ...
      if (!cancelled && browse && this.canBrowse()) {
        moveArchive(e);
        if (this.archivePull.active) {
          const open = this.archivePull.ready;
          // Keep the last displayed height through release / pointer capture
          // cleanup, then let the existing extraction or return spring finish.
          this.lift.value = this.archivePull.height;
          this.lift.velocity = 0;
          const cell = { ...this.selectedCell };
          reset();
          if (open) this.onOpen?.(this.selectedIndex, cell);
          return;
        }
        if (this.archiveDrag.active) {
          ...
          const cell = this.pickCell(e.clientX, e.clientY);
          const open = cell && sameCell(cell, this.selectedCell) && this.onOpen;
          reset();
          if (cell) {
            if (open) this.onOpen?.(fileAtCell(cell), cell);
            else this.onSelect?.(fileAtCell(cell), cell);
          }
          return;
        }
      } else if (!cancelled && !browse && !moved && this.canInspect && this.onOpen) {
        const cell = this.pickCell(e.clientX, e.clientY);
        reset();
        if (cell && sameCell(cell, this.selectedCell)) this.onOpen(this.selectedIndex, cell);
        return;
      }
      reset();
    }, { signal: this.inputEvents.signal });
  ...
  update(
    ...
    if (this.looping && !cinematic && !this.holdingArchive && !this.archiveMomentum && !this.archivePreview) this.rebaseCoordinates();
    ...
    if (!this.holdingArchive && !momentum && !this.archivePreview) {
    ...
    if (preview && !cinematic) {
      const duration = .75, t = this.reduced ? 1 : Math.min(1, (time - preview.started) / duration);
      const blend = t * t * (3 - 2 * t), tangent = t * (1 - t) ** 2;
      this.columnCamera.value = preview.x + (preview.targetX - preview.x) * blend + preview.vx * duration * tangent;
      this.rail.value = preview.z + (preview.targetZ - preview.z) * blend + preview.vz * duration * tangent;
      this.columnCamera.velocity = (preview.targetX - preview.x) * 6 * t * (1 - t) / duration + preview.vx * (3 * t * t - 4 * t + 1);
      this.rail.velocity = (preview.targetZ - preview.z) * 6 * t * (1 - t) / duration + preview.vz * (3 * t * t - 4 * t + 1);
      if (t === 1) { this.archivePreview = undefined; this.onPreviewCommit?.(preview.index, preview.cell); }
    }
    if (momentum) {
    ...
    if (!cinematic) {
      if (this.archivePull.active) {
        ...
        this.lift.velocity = 0;
        this.lastInteraction = time;
      } else if (!this.targetDetail && this.openBlend > 0) {
        // Reassemble at the current elevation before the file can descend.
        this.returnY ??= this.model.position.y;
        this.lift.value = this.returnY - selectedBase;
        this.lift.velocity = 0;
      } else if (this.returnY !== null && this.rotation !== 0) {
        this.lift.value = this.returnY - selectedBase;
        this.lift.velocity = 0;
      } else {
        this.returnY = null;
    ...
    if (!this.archivePull.active && this.lift.value <= .401) this.manualExtraction = false;
    const cameraTarget = this.manualExtraction && !this.targetDetail ? 0 : this.targetDetail
    ...
    for (let i = this.outgoing.length - 1; i >= 0; i--) {
      ...
      if (o.returnY !== null) {
        ...
        o.lift.velocity = 0;
        if (o.group.rotation.y === 0 && !o.openBlend) o.returnY = null;
      ...
      // At this point the label opacity is below 3% and the interior has
      // disappeared behind the neighboring cassettes. Keep the actual spring
      // in the shared batch instead of drawing a complete hierarchy for seconds.
      if (!o.batched && o.lift.value < .06 && !o.returnY && !o.openBlend &&
          Math.abs(o.group.rotation.y) < .0001 && o.clarity < .001) {
        const group = new THREE.Group();
        group.position.copy(o.group.position); group.rotation.copy(o.group.rotation);
        this.disposeReturning(o); o.group = group; o.deck = undefined; o.batched = true;
      }
    ...
    if (!cinematic && this.sessionDeckFocus > 0)
      viewDirection.lerp(new THREE.Vector3(0, 0, 1), this.sessionDeckFocus).normalize();
    if (cinematic) {
    ...
    if (!cinematic && this.deck && this.sessionDeckFocus > 0) {
      ...
      this.model.updateMatrixWorld(true);
      const screenAim = this.deck.group.localToWorld(new THREE.Vector3(...this.deck.screenSize.center));
      // A closer operating view retains the model's bezel, compact brand and
    ...
    if (!cinematic && !this.reduced && !this.uiOnlyParallax) {
      cameraPosition.x += this.pointer.x * 0.12 * (1 - this.sessionDeckFocus);
      cameraPosition.y -= this.pointer.y * 0.12 * (1 - this.sessionDeckFocus);
    }
    const cameraBlend = cinematic || (this.reduced && this.deckState !== "off") ? 1 : 1 - Math.exp(-dt * 5);
    ...
    this.camera.fov = THREE.MathUtils.lerp(
      ...
      THREE.MathUtils.radToDeg(
        2 * Math.atan(cameraSpan / (2 * distance)),
      ),
      cameraBlend,
    );
    ...
    const returning = new Map(this.outgoing.filter(o => o.batched).map(o => [cellKey(o.cell), o]));
    const hidden = new Set(this.outgoing.filter(o => !o.batched).map(o => cellKey(o.cell)));
    ...
    for (const item of this.outgoing) {
      ...
      item.group.visible = !item.batched && (Boolean(item.openBlend) || item.returnY !== null || this.visibility.intersects(p.x, p.y, p.z));
    }
    ...
    this.canInspect =
      !cinematic &&
      this.deckState !== "open" && this.openBlend === 0 &&
  ...
  /**
   * Desktop SSH: drive the detail reveal from a real connection timeline
   * (original-film seconds) instead of the reference film's own clock. Callers
   * only ever move it to a milestone a real event asked for, so a connection
   * that stops moving stops the animation. Pass `null` to restore the default.
   */
  ...
  getStats() {
    ...
    return {
      ...
      selectedLane: fileLocation(this.selectedIndex).lane,
      ...
      archivePull: { active: this.archivePull.active, progress: this.archivePull.progress, ready: this.archivePull.ready },
      ...
      returningAppearance: this.outgoing.map((o) => ({
        ...
        terminal: Boolean(o.deck),
        contentKey: o.deck?.group.userData.contentKey,
        cardId: o.deck?.group.userData.cardId,
      })),
